...
def upload_file():
    if 'file' not in request.files:
        return jsonify({'success': False, 'error': '没有文件被上傳'})
...
def get_session_time_remaining():
    ...
    if now >= expire_time:
        ...
        try:
            ...
            logger.error(f"清理過期会话数据失败: {str(e)}")
...
def filter_dataframe(df, params):
    ...
    # 時間筛选
    if params['year']:
        query_df = query_df[query_df['交易時間'].dt.year == params['year']]
    if params['month']:
        query_df = query_df[query_df['交易時間'].dt.month == params['month']]
...
def group_data(df, group_by):
    """根据指定方式分组数据"""
    if group_by == 'date':
        return df.groupby(df['交易時間'].dt.strftime('%Y-%m-%d'))
    elif group_by == 'category':
        return df.groupby('交易分类')
    elif group_by == 'hour':
        return df.groupby(df['交易時間'].dt.hour)
...
def get_yearly_stats():
    ...
    try:
        ...
        logger.error(f"處理年度数据時出错: {str(e)}")
        return jsonify({'error': str(e)}), 500
...
def calculate_stats(groups):
    ...
    stats = groups.agg({
        '金额': ['sum', 'count', 'mean'],
        '交易時間': lambda x: x.dt.date.nunique()
    })
...
def build_response(df, stats):
    ...
    return {
        'metadata': {
            'total_records': len(df),
            'date_range': {
                'start': df['交易時間'].min().strftime('%Y-%m-%d'),
                'end': df['交易時間'].max().strftime('%Y-%m-%d')
            }
        },
        'summary': {
            ...
            'active_days': int(df['交易時間'].dt.date.nunique())
        },
        'groups': {
            str(name): {
